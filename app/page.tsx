    ...
    <div className="space-y-10">
      <div>
        ...
        <p className="mt-2 text-sm text-zinc-500">
          Hver testside logger til Umami via{" "}
          <code className="font-mono bg-zinc-100 px-1 rounded">sporing.js</code>. URL-stien
          identifiserer hva som testes i BigQuery.
        </p>
      </div>
      ...
      {ROUTES.map(({ group, color, badge, items }) => (
        <section key={group} className="space-y-3">
          <div className="flex items-center gap-2">
            <h2 className="text-xs font-semibold text-zinc-500 uppercase tracking-wide">{group}</h2>
            {badge && (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {items.map(({ href, label, desc }) => (
              ...
              >
                <div className={`inline-block text-xs font-mono px-2 py-0.5 rounded mb-2 transition-colors ${color}`}>
                  {label}
                </div>
                <p className="text-xs text-zinc-500 group-hover:text-zinc-700 transition-colors">{desc}</p>
      ...
      <div className="border border-zinc-200 rounded-lg p-4 bg-zinc-50 text-xs font-mono text-zinc-500 space-y-1">
        <p className="font-semibold text-zinc-600">BQ — verifiser sporing</p>
        <p>SELECT * FROM umami.public_event WHERE url_path LIKE &apos;%/sporing/track%&apos; ORDER BY created_at DESC LIMIT 20;</p>
        <p>SELECT * FROM umami.public_session WHERE distinct_id IS NOT NULL ORDER BY created_at DESC LIMIT 20;</p>
      </div>
    </div>
  );
}
